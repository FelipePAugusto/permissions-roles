        ...
        <div class="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
            ...
            <div class="flex space-x-4">
                ...
                @can('user_view')
                    ...
                    ]) aria-current="page">Usuários</a>
                @endcan

                @can('post_view')
                    <a href="{{route('post.index')}}" @class([
                    'rounded-md px-3 py-2 text-sm font-medium',
                    'bg-gray-900 text-white' => request()->routeIs('post.*'),
                    'text-gray-300 hover:bg-gray-700 hover:text-white' => !request()->routeIs('post.*'),
                ...
                @if(auth()->user()->hasRole('Super Admin'))
                    ...
                    'rounded-md px-3 py-2 text-sm font-medium',
                    'bg-gray-900 text-white' => request()->routeIs('post.*'),
                    'text-gray-300 hover:bg-gray-700 hover:text-white' => !request()->routeIs('role.*'),
                    ]) aria-current="page">Funções</a>
                @endif
            </div>
            </div>
        </div>
        <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
            ...
            <div x-data="{ dropdownProfile: false }" class="relative ml-3">
                ...
                <div x-show="dropdownProfile" class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
                    ...
                    <form action="{{ route('logout') }}" class="hidden" method="post" id="form-logout">
                    @csrf            
                </form>
                </div>
            </div>
